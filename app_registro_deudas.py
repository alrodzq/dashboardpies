...
st.set_page_config(
    page_title="Control de Deudas — Terapias TO",
    page_icon="📊",
    layout="wide"
)
...
with col_title:
    ...
    st.markdown(f"<p style='font-family:JetBrains Mono;font-size:.72rem;color:#5A5A5A;letter-spacing:.06em;text-transform:uppercase'>Terapia Ocupacional · Seguimiento Semanal · {datetime.now().strftime('%B %Y')}</p>", unsafe_allow_html=True)
with col_badge:
...
if uploaded:
    ...
    st.warning("👆 **Sube el archivo REGISTRO_VISITAS_TO.xlsx** en el panel izquierdo para comenzar.")
